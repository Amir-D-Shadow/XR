...
def get_gt_data(batch_size,img_info,class_info,img_path,img_shape = (640,640),standard_scale=(8000,50000)):
   ...
   while True:
      ...
      img_data = get_image_data(name_list,img_path)
...
def get_image_data(name_list,img_path):
...
def multiple_positive_labeling(y_true,class_id,xmin,ymin,xmax,ymax,xcenter,ycenter,step_w,step_h):
   ...
   wmax = int(xmax*step_w)
   hmax = int(ymax*step_h)
   ...
   while w_pos < wmax:
      ...
      while h_pos < hmax:
         ...
         h_pos = int( h_pos + step_h )

      w_pos = int( w_pos + step_w )
