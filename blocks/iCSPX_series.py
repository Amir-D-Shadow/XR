...
from CBL import CBL
from inv_res_unit_series import inv_res_unit_series




class iCSPX_series(tf.keras.Model):

   def __init__(self,iCSPX_series_info,**kwargs):
      ...
      ------ CBL_1 ------ CBL_2 ------ inv_res_unit_series * X ------ CBL_3 -----
                     |                                                             |
                     |                                                             |______
                     |                                                              ______  Concat --- CBL_5
                     |                                                             |
      ...
      #initialization
      super(iCSPX_series,self).__init__(**kwargs)

      #extract num_of_res_unit
      self.num_of_inv_res_unit_series = iCSPX_series_info["num_of_inv_res_unit_series"]
      ...
      for i in range(1,self.num_of_inv_res_unit_series+1):

         #Extract res_unit_i info
         res_unit_info = iCSPX_series_info[f"inv_res_unit_series_info"]

         #define resunit layer
         self.res_unit_seq[f"res_unit_{i}"] = inv_res_unit_series(res_unit_info)
         

      #CBL_1
      filters,kernel_size,strides,padding = iCSPX_series_info["CBL_1"]
      
      self.CBL_1 = CBL(filters,kernel_size,strides,padding)

      #CBL_2
      filters,kernel_size,strides,padding = iCSPX_series_info["CBL_2"]
      
      self.CBL_2 = CBL(filters,kernel_size,strides,padding)

      #CBL_3
      filters,kernel_size,strides,padding = iCSPX_series_info["CBL_3"]
      
      self.CBL_3 = CBL(filters,kernel_size,strides,padding)

      #CBL_4
      filters,kernel_size,strides,padding = iCSPX_series_info["CBL_4"]
      
      self.CBL_4 = CBL(filters,kernel_size,strides,padding)

      #CBL_5
      filters,kernel_size,strides,padding = iCSPX_series_info["CBL_5"]
